 ...
 * <p>
 * The colors can be customized in two ways. The first and simplest is to provide an array of colors via {@link #setSelectedIndicatorColors(int...)} and {@link #setDividerColors(int...)}. The
...
public class SlidingTabLayout extends HorizontalScrollView {
	 ...
	 *
	 * If you only require simple custmisation then you can use {@link #setSelectedIndicatorColors(int...)} and {@link #setDividerColors(int...)} to achieve similar effects.
	 */
	...
	/**
	 * Sets the colors to be used for tab dividers. These colors are treated as a circular array. Providing one color will mean that all tabs are indicated with the same color.
	 */
	...
	/**
	 * Set the {@link ViewPager.OnPageChangeListener}. When using {@link SlidingTabLayout} you are required to set any {@link ViewPager.OnPageChangeListener} through this method. This is so that the
	 * layout can update it's scroll position correctly.
	 *
	...
	private void populateTabStrip() {
		...
		for (int i = 0; i < adapter.getCount(); i++) {
			...
			tabIconView.setScaleType(ScaleType.FIT_XY);

			LinearLayout tabView = createDefaultTabView(getContext());
			
			tabView.addView(tabIconView);
	...
	private class InternalViewPagerListener implements ViewPager.OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			LinearLayout selectedTab = (LinearLayout) mTabStrip.getChildAt(position);
			selectedTab.getChildAt(0).setSelected(true);
			
			LinearLayout lastSelectedTab = (LinearLayout) mTabStrip.getChildAt(lastPosition);
			lastSelectedTab.getChildAt(0).setSelected(false);
	...
	public void initSelectedPage() {
		LinearLayout tabView = (LinearLayout) mTabStrip.getChildAt(0);
		tabView.getChildAt(0).setSelected(true);
	}

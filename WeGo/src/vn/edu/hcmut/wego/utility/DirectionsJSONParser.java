...
public class DirectionsJSONParser {
	...
	public HashMap<String, List<List<HashMap<String,String>>>> parse(JSONObject jObject){
			...
			for(int i=0;i<jRoutes.length();i++){			
				...
				for(int j=0;j<jLegs.length();j++){
					...
					for(int k=0;k<jSteps.length();k++){
						...
						if(((JSONObject)jSteps.get(k)).has("html_instructions"))
						{
							String zz = ((JSONObject)jSteps.get(k)).getString("html_instructions");
							array.add(Html.fromHtml(zz).toString());
						}
